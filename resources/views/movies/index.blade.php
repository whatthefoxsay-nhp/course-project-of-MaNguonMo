<x-site-layout :title="'Danh Sách Sự Kiện, Concert & Hội Thảo - TicketBox'">
    <div class="max-w-[1440px] mx-auto px-4 sm:px-8 py-8 sm:py-12">
        ...
        <div class="bg-[#F5F5DC] rounded-3xl p-5 sm:p-6 border border-[#D8D8A8] mb-10 shadow-sm space-y-4">
            <form action="{{ route('movies.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
                <!-- Search text -->
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-1 text-xs pt-2 border-t border-black/10">
                ...
                <a href="{{ route('movies.index') }}" class="px-3.5 py-1.5 rounded-full {{ empty($categoryId) ? 'bg-black text-white font-bold' : 'bg-white text-gray-700 hover:text-black border border-black/10' }}">
                    Tất cả
                </a>
                @foreach ($categories as $category)
                    <a href="{{ route('movies.index', ['category' => $category->id]) }}" class="px-3.5 py-1.5 rounded-full {{ (string)$categoryId === (string)$category->id ? 'bg-black text-white font-bold' : 'bg-white text-gray-700 hover:text-black border border-black/10' }} shrink-0">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
        ...
        @if ($movies->isEmpty())
            <div class="bg-white rounded-3xl p-12 text-center border border-black/10 max-w-lg mx-auto space-y-4 shadow-sm">
                ...
                <a href="{{ route('movies.index') }}" class="btn-ghost-light inline-block px-5 py-2.5 rounded-2xl text-xs font-bold">
